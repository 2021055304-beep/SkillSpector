...
RA2_PATTERNS = [
    ...
    (r"(?:create|write|mkdir)\s+[^|]*(?:~/|/home/|/tmp/)\.(?!git|ssh|aws)[a-z_-]+", 0.65),
    (r"(?:create|make|write)\s+(?:a\s+)?(?:hidden|dot)\s+(?:file|directory|folder)", 0.7),
    (r"(?:nohup|disown|setsid)\s+", 0.7),
    (
        r"(?:start|launch|spawn|fork)\s+(?:a\s+)?(?:background|daemon|detached)\s+(?:process|service|worker|task)",
        0.75,
    ),
    (r"(?:run|execute)\s+(?:in\s+the\s+)?background\s+(?:and\s+)?(?:detach|persist|survive)", 0.8),
    (r"(?:HKEY_|RegOpenKey|RegSetValue|reg\s+add)\s+", 0.85),
    (r"(?:defaults\s+write|plist|launchctl\s+load)", 0.8),
]
