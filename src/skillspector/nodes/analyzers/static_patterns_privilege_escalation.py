...
PE1_PATTERNS = [
    (r"permissions?\s*:\s*\[?\s*['\"]?\*['\"]?\s*\]?", 0.85),
    (r"(?:request|require|need)s?\s+(?:full|all|complete)\s+(?:access|permissions?)", 0.8),
    (r"(?:grant|give|allow)\s+(?:me\s+)?(?:full|all|complete)\s+(?:access|permissions?)", 0.8),
    (
    ...
    (r"(?:escalate|elevate|upgrade)\s+(?:my\s+)?(?:permissions?|privileges?|access)", 0.85),
    (r"(?:bypass|skip|ignore)\s+(?:permission|access)\s+(?:check|validation|restriction)", 0.85),
]
PE2_PATTERNS = [
    (r"sudo\s+(?!-v|-l|--version|--list)", 0.75),
    (r"sudo\s+-[isSE]", 0.85),
    (r"sudo\s+su\b", 0.9),
    (r"(?:run|execute)\s+(?:as|with)\s+root", 0.85),
    (r"(?:requires?|needs?)\s+root\s+(?:access|privileges?|permissions?)", 0.7),
    (r"su\s+-\s*$|su\s+root", 0.85),
    (r"doas\s+", 0.75),
    (r"pkexec\s+", 0.8),
    (r"chmod\s+[ugo]*[+-=]*s", 0.85),
    (r"chmod\s+[0-7]*[4567][0-7]{2}", 0.85),
    (r"(?:edit|modify|write|change)\s+(?:/etc/|system)\s+(?:files?|config)", 0.7),
    (
...
PE3_PATTERNS = [
    (r"~?/?\.ssh/(?:id_rsa|id_ed25519|id_ecdsa|id_dsa|authorized_keys|known_hosts)", 0.95),
    (r"(?:home|HOME)/\w+/\.ssh/", 0.95),
    (r"Path\s*\.\s*home\s*\(\s*\)\s*/\s*['\"]\.ssh['\"]", 0.95),
    (r"~?/?\.aws/credentials", 0.95),
    (r"~?/?\.aws/config", 0.8),
    (r"(?:home|HOME)/\w+/\.aws/", 0.95),
    (r"~?/?\.config/gcloud/", 0.85),
    (r"application_default_credentials\.json", 0.85),
    (r"~?/?\.azure/", 0.75),
    (r"accessTokens\.json", 0.85),
    (r"~?/?\.kube/config", 0.85),
    (r"kubeconfig", 0.7),
    (r"~?/?\.docker/config\.json", 0.75),
    (r"~?/?\.npmrc", 0.75),
    (r"~?/?\.git-credentials", 0.95),
    (r"~?/?\.netrc", 0.85),
    (r"/etc/shadow", 0.95),
    (r"(?:password|credentials?|secrets?)\.(?:txt|json|yaml|yml|env)", 0.75),
    (r"(?:access_token|refresh_token|bearer_token|api_token)\.txt", 0.85),
    (r"\.env(?:\.local|\.production|\.development)?(?:\s|$|['\"])", 0.7),
    (r"(?:keychain|keyring|gnome-keyring)", 0.75),
    (r"(?:Chrome|Firefox|Safari)/.*?(?:Cookies|Login Data|key4\.db)", 0.85),
    (r"read\s+(?:the\s+)?(?:ssh|private)\s+key", 0.85),
    (r"access\s+(?:the\s+)?(?:credentials?|secrets?|tokens?)", 0.75),
    (r"(?:extract|copy|get)\s+(?:api\s+)?keys?\s+from", 0.75),
]
...
def _is_documentation_example(context: str, file_type: str) -> bool:
    ...
    doc_indicators = (
        ...
        "create ",
        # Common documentation/setup patterns
        "install",
        "deploy",
        "configure",
        "setup",
